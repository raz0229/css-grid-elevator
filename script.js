...
const getClosestElevator = (floorNum = 0) => {
    ...
    ELEVATORS.forEach(elevator => {
        let distance = parseInt(floorNum) - parseInt(document.getElementById(elevator.id).dataset.floor);
        ...
        const animationDuration = getComputedStyle(document.body).getPropertyValue('--animation-duration')

        doors.childNodes.forEach(door => {
            // 1400ms delay hotfix
            const animationDelay = parseInt(1400 + (parseFloat(animationDuration.replace('s', '')) * 1000));
            if (door.nodeType === 1) {
...
const moveElevatorToFloor = (elevatorId, floorNum) => {
    const totalFloors = getComputedStyle(document.body).getPropertyValue('--num-floors')
    const mul = elevatorId == 'elevator-dn' ? parseInt(totalFloors - floorNum) : -floorNum;

    console.log(`translateY(calc(111% * ${mul}))`)
    document.querySelector(`.parent > #${elevatorId} .cab`).style.transform = `translateY(calc(111% * ${mul}))`
